...
NanoThermEMSolver::NanoThermEMSolver(ParMesh & pmesh, int order,
...
NanoThermEMSolver::~NanoThermEMSolver()
{
   delete epsCoef_;
   delete muInvCoef_;
   delete etaInvCoef_;
...
HYPRE_BigInt
NanoThermEMSolver::GetProblemSize()
{
   return HCurlFESpace_->GlobalTrueVSize();
}

void
NanoThermEMSolver::PrintSizes()
{
...
void
NanoThermEMSolver::SetInitialEField(VectorCoefficient & EFieldCoef)
{
   eCoef_ = &EFieldCoef;
...
void
NanoThermEMSolver::SetInitialBField(VectorCoefficient & BFieldCoef)
{
   bCoef_ = &BFieldCoef;
...
void
NanoThermEMSolver::Mult(const Vector &B, Vector &dEdt) const
{
   implicitSolve(0.0, B, dEdt);
}

void
NanoThermEMSolver::ImplicitSolve(double dt, const Vector &B, Vector &dEdt)
{
   implicitSolve(dt, B, dEdt);
}

void
NanoThermEMSolver::setupSolver(const int idt, const double dt) const
{
...
void
NanoThermEMSolver::implicitSolve(double dt, const Vector &B, Vector &dEdt) const
{
...
void
NanoThermEMSolver::SyncGridFuncs()
{
   e_->Distribute(*E_);
   b_->Distribute(*B_);
}
...
double
NanoThermEMSolver::GetMaximumTimeStep() const
{
   if ( dtMax_ > 0.0 )
   {
      return dtMax_;
...
double
NanoThermEMSolver::GetEnergy() const
{
   double energy = 0.0;
...
void
NanoThermEMSolver::RegisterVisItFields(VisItDataCollection & visit_dc)
{
   visit_dc_ = &visit_dc;
...
void
NanoThermEMSolver::WriteVisItFields(int it)
{
   if ( visit_dc_ )
   {
      if ( myid_ == 0 && logging_ > 1 )
...
void
NanoThermEMSolver::InitializeGLVis()
{
   if ( myid_ == 0 && logging_ > 0 )
...
void
NanoThermEMSolver::DisplayToGLVis()
{
   if ( myid_ == 0 && logging_ > 1 )
