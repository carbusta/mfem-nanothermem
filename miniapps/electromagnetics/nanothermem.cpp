...
//
// -------------------------------------------------------------------------------------
// NanoThermEM Miniapp:  Full-Wave Electromagnetics and Thermodynamics of Nanostructures
// -------------------------------------------------------------------------------------
//
///////////////////////////////////////////////////////////////////
// Below is the description for the Maxwell code, WIP for this code
///////////////////////////////////////////////////////////////////
//
...
#include "nanothermem_solver.hpp"
#include <fstream>
#include <iostream>
...
static Vector db_params_(0);  // Radius, Half-Opening Angle, Real and Imag Permittivity
//                               of dielectric nanoblade
static Vector dt_params_(0);  // Radius, Half-Opening Angle, Real and Imag Permittivity
//                               of dielectric nanotip
double dielectric_sphere(const Vector &);
double dielectric_blade (const Vector &);
double dielectric_tip   (const Vector &);
double epsilon(const Vector &x) { 
    if(ds_params_.Size() > 0)
        return dielectric_sphere(x);
    else if(db_params_.Size() > 0)
        return dielectric_blade(x);
    else if(dt_params_.Size() > 0)
        return dielectric_tip(x);
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&db_params_, "-db", "--dielectric-blade-params",
                  "Radius, Half-Opening Angle, Real and Imaginary Permittivity of Dielectric Nanoblade");
   args.AddOption(&dt_params_, "-db", "--dielectric-tip-params",
                  "Radius, Half-Opening Angle, Real and Imaginary Permittivity of Dielectric Nanotip");
   ...
   NanoThermEMSolver NanoThermEM(pmesh, sOrder,
                         ( ds_params_.Size() > 0  || db_params_.Size() > 0 || dt_params_.Size() > 0 ) ? epsilon     : NULL,
   ...
   NanoThermEM.SetInitialEField(EFieldCoef);
   NanoThermEM.SetInitialBField(BFieldCoef);

   // Compute the energy of the initial fields
   double energy = NanoThermEM.GetEnergy();
   if ( Mpi::Root() )
   {
   ...
   double dtmax = NanoThermEM.GetMaximumTimeStep();
   ...
   siaSolver.Init(NanoThermEM.GetNegCurl(), NanoThermEM);


   // Initialize GLVis visualization
   if (visualization)
   {
      NanoThermEM.InitializeGLVis();
   }

   // Initialize VisIt visualization
   VisItDataCollection visit_dc("NanoThermEM-Parallel", &pmesh);

   double t = ti;
   NanoThermEM.SetTime(t);

   if ( visit )
   {
      NanoThermEM.RegisterVisItFields(visit_dc);
   }

   // Write initial fields to disk for VisIt
   if ( visit )
   {
      NanoThermEM.WriteVisItFields(0);
   }

   // Send the initial condition by socket to a GLVis server.
   if (visualization)
   {
      NanoThermEM.DisplayToGLVis();
   }
   ...
   while (t < tf)
   {
      ...
      siaSolver.Run(NanoThermEM.GetBField(), NanoThermEM.GetEField(), t, dt,
                    max(t + dt, ti + ts * it));

      // Approximate the current energy if the fields
      energy = NanoThermEM.GetEnergy();
      if ( Mpi::Root() )
      {
      ...
      NanoThermEM.SyncGridFuncs();

      // Write fields to disk for VisIt
      if ( visit )
      {
         NanoThermEM.WriteVisItFields(it);
      }
...
// Print the NanoThermEM ascii logo to the given ostream
void display_banner(ostream & os)
{
   os << "     ___    ____                                      " << endl
      << "    /   |  /   /                           __   __    " << endl
...
// A nanoblade (wedge with curved apex). The nanoblade is centered at the origin.
// It has a radius, half-opening angle, and a complex permittivity (to be implemented)
double dielectric_blade(const Vector &x)
{
    MFEM_ASSERT(x.Size() >= 2, "blade structure requires at least 2 dimensions.");
    //       within apex cylinder OR
    //       within wedge AND not above wedge-cylinder boundary intersections
    //       x^2 + y^2 < R^2 || ( |x| < -y*tan(t) + R(sin(t)tan(t)+cos(t)) && y < Rsin(t) )
    if ( (x(0)*x(0) + x(1)*x(1) <= db_params_(0)*db_params_(0)) ||
         ( (abs(x(0)) <= -x(1)*tan(db_params_(1)) + db_params_(0)*(sin(db_params_(1))*tan(db_params_(1))+cos(db_params_(1)))) && 
         (x(1) <= db_params_(0)*sin(db_params_(1))) ) ){
        return db_params_(2) * epsilon0_; //real component, need to do imag
    }
    return epsilon0_;
}

double dielectric_tip(const Vector &x)
{
    MFEM_ASSERT(x.Size() >= 3, "tip structure requires at least 3 dimensions.");
    if( dt_params_(1) == 0.0 ){ //cylinder body
        //      within apex sphere OR
        //      within cylinder AND below sphere center
        //      x^2 + y^2 + z^2 < R^2 || ( x^2 + y^2 < R^2 && z < 0 )
        if (x(0)*x(0) + x(1)*x(1) + x(2)*x(2) <= dt_params_(0)*dt_params_(0) ||
        (   x(0)*x(0) + x(1)*x(1) <= dt_params_(0)*dt_params_(0) && x(2) <= 0.0  ) ){
            return dt_params_(2) * epsilon0_; //real component, need to do imag
        }
    }
    else{ //cone body
        //      within apex sphere OR
        //      within cone AND not above sphere-cone boundary intersections
        //      x^2 + y^2 + z^2 < R^2 || ( sqrt(x^2+y^2) < -z*tan(t) + R(sin(t)tan(t)+cos(t)) && z < Rsin(t) )
        if ( (x(0)*x(0) + x(1)*x(1) + x(2)*x(2) <= dt_params_(0)*dt_params_(0)) ||
             ( (sqrt(x(0)*x(0) + x(1)*x(1)) <= -x(2)*tan(dt_params_(1)) + dt_params_(0)*(sin(dt_params_(1))*tan(dt_params_(1))+cos(dt_params_(1)))) && 
             (x(2) <= dt_params_(0)*sin(dt_params_(1))) ) ){
            return dt_params_(2) * epsilon0_; //real component, need to do imag
        }
    }
    return epsilon0_;
